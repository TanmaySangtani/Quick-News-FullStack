...
const UserSavedArticles = () => {
  ...
  const context = useContext(contextAddress)
  const { checkLogin, getSavedArticles, whenImageNull, removeSaveArticle } = context
  const [artlist, setArtlist] = useState([])
  const [loading, setLoading] = useState(true)

  const fetchArts = async () => {
    // console.log(await getSaveArticles())
    const arr = await getSavedArticles()
    setArtlist(arr)
    setLoading(false)
  }

  const onClickRemove = async (articleId) => {
    const result = await removeSaveArticle(articleId)
  ...
  return (
    <>
      <div style={{ marginTop: '6rem', display: 'flex', flexDirection: 'column' }}>
        <h1 style={{ fontFamily: 'Times New Roman', textAlign: 'center' }}>Saved Articles</h1>
